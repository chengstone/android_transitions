 ...
 * {@link R.styleable#Transition_Scale Scale Attributes}
 *
...
public class Scale extends Visibility {
	 ...
	 */
	@VisibleForTesting static final String PROPERTY_TRANSITION_SCALE_X = Scale.class.getName() + ":transition.scaleX";

	/**
	 * Name of the property holding scale Y value for animating view in {@link TransitionValues}.
	 */
	@VisibleForTesting static final String PROPERTY_TRANSITION_SCALE_Y = Scale.class.getName() + ":transition.scaleY";
	...
	/**
	 * X coordinate for scaling pivot of the animating view. If {@code null}, {@link #pivotXFraction}
	 * should be used to calculate this coordinate.
	 */
	private Float pivotX;

	/**
	 * Y coordinate for scaling pivot of the animating view. If {@code null}, {@link #pivotYFraction}
	 * should be used to calculate this coordinate.
	 */
	 ...
	 * pivot of the animating view if {@link #pivotX} coordinate has not been specified.
	 */
	private float pivotXFraction = SCALE_FRACTION;

	/**
	 * Fraction from the {@code [0.0, 1.0]} range that can be used to calculate Y coordinate for scaling
	 * pivot of the animating view if {@link #pivotY} coordinate has not been specified.
	 */
	private float pivotYFraction = SCALE_FRACTION;
	...
	public Scale(@NonNull final Context context, @Nullable final AttributeSet attrs) {
		super(context, attrs);
		final TypedArray attributes = context.obtainStyledAttributes(attrs, R.styleable.Transition_Scale, 0, 0);
		...
		for (int i = 0; i < attributeCount; i++) {
			...
			if (attrIndex == R.styleable.Transition_Scale_android_pivotX) {
				this.pivotXFraction = attributes.getFraction(attrIndex, 1, 1, pivotXFraction);
			} else if (attrIndex == R.styleable.Transition_Scale_android_pivotY) {
				this.pivotYFraction = attributes.getFraction(attrIndex, 1, 1, pivotYFraction);
			} else if (attrIndex == R.styleable.Transition_Scale_android_transformPivotX) {
				this.pivotX = (float) attributes.getDimensionPixelSize(attrIndex, 0);
			} else if (attrIndex == R.styleable.Transition_Scale_android_transformPivotY) {
				this.pivotY = (float) attributes.getDimensionPixelSize(attrIndex, 0);
			}
		}
		attributes.recycle();
	}
	 ...
	 * @see #setPivotX(Float)
	 */
	@Nullable public Float getPivotX() {
		return pivotX;
	}
	 ...
	 * @see #setPivotY(Float)
	 */
	@Nullable public Float getPivotY() {
		return pivotY;
	}
	...
	public void setPivotXFraction(@FloatRange(from = 0, to = 1) final float fractionX) {
		this.pivotXFraction = Math.max(0, Math.min(1, fractionX));
	}
	 ...
	 * @see #setPivotXFraction(float)
	 */
	@FloatRange(from = 0, to = 1) public float getPivotXFraction() {
		return pivotXFraction;
	}
	...
	public void setPivotYFraction(@FloatRange(from = 0, to = 1) final float fractionY) {
		this.pivotYFraction = Math.max(0, Math.min(1, fractionY));
	}
	 ...
	 * @see #setPivotYFraction(float)
	 */
	@FloatRange(from = 0, to = 1) public float getPivotYFraction() {
		return pivotYFraction;
	}
	 ...
	 */
	@VisibleForTesting
	@Size(2) static float[] obtainStartScales(final TransitionValues startValues, final float defaultStartX, final float defaultStartY) {
	...
	@VisibleForTesting void calculateTransitionProperties(final View view) {
		info.pivotX = pivotX == null ? (view.getWidth() * pivotXFraction) : pivotX;
		info.pivotY = pivotY == null ? (view.getHeight() * pivotYFraction) : pivotY;
	}
