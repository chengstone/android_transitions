...
public final class ScaleTest extends ViewTransitionTestCase {
	...
	public void testInstantiationWithAttributeSet() {
		assertThat(new Scale(mApplication, null).getMode(), is(Scale.MODE_IN | Scale.MODE_OUT));
	}
	...
	public void testOnAppearWithSameStartAndEndValues() {
		...
		final Animator animator = scale.onAppear(new FrameLayout(mApplication), view, startValues, null);
		assertThatAnimatorIsValid(animator, view, 0.0f, 0.0f);
	...
	public void testOnDisappearWithSpecifiedPivotsFraction() {
		...
		view.setTop(0);
		view.setBottom(100);
		final Animator animator = scale.onDisappear(new FrameLayout(mApplication), view, new TransitionValues(), new TransitionValues());
		assertThatAnimatorIsValid(animator, view, 1.0f, 1.0f);
