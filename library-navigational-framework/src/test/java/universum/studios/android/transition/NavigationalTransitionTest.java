 ...
 */
public final class NavigationalTransitionTest extends RobolectricTestCase {
	...
	@Test public void testInstantiationWithActivityClass() {
		// Act:
		final TestTransition navTransition = new TestTransition(TestActivity.class);
		// Assert:
		assertSame(navTransition.getActivityClass(), TestActivity.class);
	}
	...
	@Test public void testStartForFragmentAtKitKatApiLevel() {
		...
		final NavigationalTransition navTransition = new TestTransition(SecondaryTestActivity.class).exitTransition(transition);
		// Act:
	...
	@Test public void testOnStartForFragmentAtKitKatApiLevel() {
		...
		fragmentManager.beginTransaction().add(fragment, null).commit();
		fragmentManager.executePendingTransactions();
		final NavigationalTransition navTransition = new TestTransition(SecondaryTestActivity.class);
		// Act:
	...
	@Config(sdk = Build.VERSION_CODES.KITKAT)
	@Test public void testOnStartForFragmentWithRequestCodeAtKitKatApiLevel() {
		// Arrange:
		final Activity activity = Robolectric.buildActivity(TestActivity.class).create().start().resume().get();
		final FragmentManager fragmentManager = activity.getFragmentManager();
		final TestFragment fragment = new TestFragment();
		fragmentManager.beginTransaction().add(fragment, null).commit();
		fragmentManager.executePendingTransactions();
		final NavigationalTransition navTransition = new TestTransition(SecondaryTestActivity.class).requestCode(100);
		// Act:
		navTransition.onStart(fragment);
		// Assert:
		assertThat(activity.isFinishing(), is(false));
	...
	@Test public void testFinishForFragment() {
		...
		fragmentManager.beginTransaction().add(fragment, null).commit();
		fragmentManager.executePendingTransactions();
		final NavigationalTransition navTransition = new TestTransition();
		// Act:
